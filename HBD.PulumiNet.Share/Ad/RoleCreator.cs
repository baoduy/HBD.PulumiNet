...
{
    public record Args(Environments Env, string AppName,  string Name,string? ModuleName=default, string Location = "GLB",
        Input<string>[]? Members = default, GroupCreator.GroupPermissionArgs[]? Permissions = null);
    ...
    public static Task<Group> Create(Args args)
    {
        ...
        return GroupCreator.Create(new GroupCreator.Args(name,args.Members,args.Permissions));
    }
}